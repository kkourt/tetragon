...
func loadCgTrackerSensor(t *testing.T) *sensors.Sensor {
	progs := []*program.Program{MkdirProg(), ReleaseProg()}
	m := program.MapBuilder(MapName, progs...)
	m.SetMaxEntries(MapEntries)
	s := &sensors.Sensor{
		Name:  "cgtracker_test",
		Progs: progs,
		Maps:  []*program.Map{m},
...
func doTest(t *testing.T, cgfsPath string) {
	...
	// NB(kkourt): cgtracker hooks into _release() not _rmdir() which seems to run under
	// kworker after the rmdirs() are completed. If we see failures here, we might consider
	// retryhing.
...
func TestCgTracker(t *testing.T) {
	loadCgTrackerSensor(t)
	doTest(t, "/sys/fs/cgroup")
}
